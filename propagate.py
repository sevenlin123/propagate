...
class propagate:
    ...
    def __init__(self, a, e, i, arg, node, M0, epoch, obs_date, helio=False):
        self.u = 2.9630927492415936E-04 # standard gravitational parameter 
        #self.u = 0.00029591220650534517
        self.epsilon =  23.43929 * np.pi/180. # obliquity
        #self.epsilon = 23.4368883003 * np.pi/180.
        if helio:
        ...
        self.M = (self.u/self.a**3)**0.5 * (self.obs_date - self.epoch) + self.M0
        self.X, self.Y, self.Z, self.VX, self.VY, self.VZ = self.kep_to_xyz(self.a, self.e, self.i, self.arg, self.node, self.M)
    ...
    def xyz_to_kep(self, X, Y, Z, VX, VY, VZ):
        # compute the barycentric distance r
        r = (X**2 + Y**2 + Z**2)**0.5
        ...
        ex = (VY * hz - VZ * hy)/self.u - X/r
        ey = (VZ * hx - VX * hz)/self.u - Y/r
        ez = (VX * hy - VY * hx)/self.u - Z/r
        ...
        i = np.arccos(hz/h)
        # compute eccentric anomaly E
        E = 2*np.arctan(np.tan(v/2)/((1+e)/(1-e))**0.5)
        ...
        M[M<0] += 2*np.pi
        # compute a
        a = 1/(2/r - (VX**2+VY**2+VZ**2)/self.u)
        return a, e, i, arg, node, M   
        
    def helio_to_bary(self, a, e, i, arg, node, M, epoch):
        # This is heliocentric xyz postion
        
        X0, Y0, Z0, VX0, VY0, VZ0 =  self.kep_to_xyz(a, e, i, arg, node, M)
        ...
        #print(x_sun,y_sun * np.cos(-self.epsilon) - z_sun * np.sin(-self.epsilon),y_sun * np.sin(-self.epsilon) + z_sun * np.cos(-self.epsilon), vx_sun,vy_sun * np.cos(-self.epsilon) - vz_sun * np.sin(-self.epsilon),vy_sun * np.sin(-self.epsilon) + vz_sun * np.cos(-self.epsilon))
        #print((x_sun**2+y_sun**2+z_sun**2)**0.5, (vx_sun**2+vy_sun**2+vz_sun**2)**0.5)
        ...
        print(vx_sun, (vy_sun * np.cos(-self.epsilon) - vz_sun * np.sin(-self.epsilon)), ( vy_sun * np.sin(-self.epsilon) + vz_sun * np.cos(-self.epsilon)))
        #print(VX0**2+VY0**2+VZ0**2)
        print(X-X0,Y-Y0,Z-Z0,VX-VX0,VY-VY0,VZ-VZ0)
        # transfer back to keplerian elements
        a, e, i, arg, node, M = self.xyz_to_kep(X, Y, Z, VX, VY, VZ)
        return a, e, i, arg, node, M
    ...
    def xyz_to_equa(self, X0, Y0, Z0, epoch):
        ...
        for i in range(3): 
            ...
            ltt = delta / c
            M = (self.u/self.a**3)**0.5 * (-ltt) + self.M
            X0, Y0, Z0, VX0, VY0, VZ0 = self.kep_to_xyz(self.a, self.e, self.i, self.arg, self.node, M)
        #print(X,Y,Z)    
...
if __name__ == '__main__':
    ...
    GH137_helio = np.array([3.979291654743955E+01, 2.336699438884731E-01, 1.344241830596374E+01*np.pi/180., 1.536827090353943E+02*np.pi/180., 3.572856612008427E+01*np.pi/180., 2.338003676460184E+01*np.pi/180., 2458285.5, 2458285.5])
