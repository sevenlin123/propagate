...
class propagate_lite(propagate):
    """
    faster, inaccurate version of propagate
    input Keplerian elements in the format of numpy arrays
    Units: au, radian, JD
    ...
    def __init__(self, a, e, i, arg, node, M0, epoch, obs_date, helio=False):
        ...
        self.M = (self.u_bary/self.a**3)**0.5 * (self.obs_date - self.epoch) + self.M0
        self.X, self.Y, self.Z, self.VX, self.VY, self.VZ = self.kep_to_xyz(self.a, self.e, self.i, self.arg, self.node, self.M, self.u_bary)
        self.ra, self.dec, self.r, self.delta = self.xyz_to_equa(self.X, self.Y, self.Z, self.obs_date)
 
    def calcE(self, e, M, N):
        E = M
        for kk in range(N):
            E = M + e * np.sin(E)
        return E
...
if __name__ == '__main__':
    ...
    BP519 = np.array([4.490784528803734E+02, 9.215119038599956E-01, 5.411067896592579E+01*np.pi/180., 3.480604843629308E+02*np.pi/180., 1.352131890380128E+02*np.pi/180., 3.584734429420708E+02*np.pi/180., 2458285.5, 2469285.5])
    ...
    p_lite = propagate_lite(object.T[0], object.T[1], object.T[2], object.T[3], object.T[4], object.T[5], object.T[6], object.T[7])
    ...
    print(p.ra*180/np.pi, p.dec*180/np.pi, p_lite.ra*180/np.pi, p.dec*180/np.pi)
